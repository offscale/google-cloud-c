...
struct StatusAndCstrAndCStr create_fw_net_instance(const struct configuration *const config, const struct InstanceIncomplete *const instance) {
    ...
    const struct OptionalInstance optionalInstance = instance_incomplete_create_all(
            instance,
            /*network_name*/ NULL,
            /*firewall_name*/ NULL
    ); /* instance_create_all */
...
struct StatusAndCstr get_instance_ip(const struct configuration *const config, const char *instance_name) {
    AUTH_CONTEXT.project_id=config->google_project_id, AUTH_CONTEXT.google_access_token=config->google_access_token;
    INSTANCE_CONTEXT.zone = config->google_zone;
...
struct StatusAndArrayCStrArray compute(const struct configuration *const config, const char *kind, bool list_only) {
    AUTH_CONTEXT.project_id=config->google_project_id, AUTH_CONTEXT.google_access_token=config->google_access_token;
    INSTANCE_CONTEXT.zone = config->google_zone;
    ...
    if (list_only) {
        if (strcmp(kind, "instances") == 0) {
            ...
            const struct StatusAndArrayCStrArray instanceNames = {EXIT_SUCCESS, _instanceNames, _instances.size};
            ...
            const struct StatusAndArrayCStrArray zoneNames = {EXIT_SUCCESS, _zoneNames, _zones.size};
            return zoneNames;
        }
    } else {
