...
const struct Instance EMPTY_INSTANCE = {
...
struct OptionalInstance instance_from_json(const JSON_Object *jsonObject) {
    ...
    {
        const JSON_Array *network_json_items = json_object_get_array(jsonObject, "networkInterfaces");
        const size_t network_json_items_n = json_array_get_count(network_json_items);
        size_t i;
        networkInterfaces = (struct NetworkInterface *) malloc(network_json_items_n * sizeof(struct NetworkInterface));
        for (i = 0; i < network_json_items_n; i++) {
            const JSON_Object *network_json = json_array_get_object(network_json_items, i);
            struct AccessConfigs *accessConfigs;
            {
                const JSON_Array *ac_json_items = json_object_get_array(network_json, "accessConfigs");
                const size_t ac_json_items_n = json_array_get_count(ac_json_items);
                size_t j;
                accessConfigs = (struct AccessConfigs *) malloc(ac_json_items_n * sizeof(struct AccessConfigs));
            ...
            const struct NetworkInterface networkInterface = {
                    ...
                    json_object_get_string(network_json, "name"),
                    accessConfigs
            };
...
struct Instances instances_list() {
    ...
    char *path;
    asprintf(&path, "/v1/projects/%s/zones/%s/instances", AUTH_CONTEXT.project_id, INSTANCE_CONTEXT.zone);
    struct ServerResponse response = gcloud_get(NULL, path, NULL);
    ...
    const JSON_Value *json_item = if_error_exit(json_parse_string(response.body), false);
    const JSON_Array *json_items = json_object_get_array(json_value_get_object(json_item), "items");
    const size_t json_items_n = json_array_get_count(json_items);

    struct Instance *instances = (struct Instance *) malloc(json_items_n * sizeof(struct Instance));
    size_t i;
    for (i = 0; i < json_items_n; i++) {
        const JSON_Object *json_obj = json_array_get_object(json_items, i);
        const struct OptionalInstance optionalInstance = instance_from_json(json_obj);
...
struct OptionalInstance instance_get(const char *instance_name) {
    ...
    char *path;
    asprintf(&path, "/v1/projects/%s/zones/%s/instances/%s", AUTH_CONTEXT.project_id, INSTANCE_CONTEXT.zone,
             instance_name);
    struct ServerResponse response = gcloud_get(NULL, path, NULL);
...
struct OptionalInstance instance_insert(const struct InstanceIncomplete *instance, const char *network_name) {
    ...
    asprintf(&body, "{\n"
                    ...
                    "      ],\n"
                    "      \"network\": "
                    "\"https://compute.googleapis.com/compute/v1/projects/%s/global/networks/%s\"\n"
                    /*",      \"subnetwork\":"
                    ...
                    "    ]\n"
                    "  }\n"
                    "}", instance->name, instance->zone, INSTANCE_CONTEXT.zone, instance->machineType,
             AUTH_CONTEXT.project_id, network_name, shell_script);
    ...
    assert((response.status_code == 200 || response.status_code == 201) && strlen(response.body) > 0);
    ...
    && strcmp(json_object_get_string(json_object, "kind"), "compute#operation") == 0) {
        const struct OptionalGoogleCloudOperation optionalGoogleCloudOperation = google_cloud_operation_from_json(
                json_object
        );
        if (optionalGoogleCloudOperation.set) {
            const struct GoogleCloudOperation googleCloudOperation = optionalGoogleCloudOperation.googleCloudOperation;
            if (strcmp(googleCloudOperation.status, "RUNNING") == 0)
                return instance_get(instance->name);
            else {
                const struct OptionalInstance optionalInstance = {false, EMPTY_INSTANCE};
                fprintf(stderr,
                        "instance_insert operation not RUNNING. Got:\n%s\n",
    ...
    const struct OptionalInstance optionalInstance = {false, EMPTY_INSTANCE};
    fprintf(stderr,
            "instance_insert final else block on %s\n",
            json_serialize_to_string_pretty(json_item));
    return optionalInstance;
}

struct OptionalInstance instance_incomplete_create_all(const struct InstanceIncomplete *instance,
                                                       const char *network_name,
                                                       const char *firewall_name) {
    /* Creates network, firewall, and instance */

    /* TODO: Proper request/response handling with structs and all for network and firewall rules */
    ...
    bool network_existent = optionalNetwork.set,
            firewall_existent = false,
    ...
    if (!firewall_existent) {
        struct OptionalFirewall optionalFirewall = firewall_create(network_name, firewall_name);
    ...
    } else optionalInstance = instance_get((*instance).name);
