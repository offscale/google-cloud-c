...
struct Zones zone_list() {
    ...
    const JSON_Value *zones_json_item = if_error_exit(json_parse_string(response.body), false);
    const JSON_Array *zone_json_items = json_object_get_array(json_value_get_object(zones_json_item), "items");
    const size_t zone_json_items_n = json_array_get_count(zone_json_items);
    size_t i;

    struct Zone *zones = (struct Zone *) malloc(zone_json_items_n * sizeof(struct Zone));
    for (i = 0; i < zone_json_items_n; i++) {
        ...
        const struct Zone zone = {
                ...
                /* creationTimestamp */ json_object_get_string(zone_json, "creationTimestamp"),
                ...
                /* availableCpuPlatforms */ NULL, //zone.object.at("availableCpuPlatforms").array,
                /* supportsPzs */ (bool) json_object_get_boolean(zone_json, "supportsPzs"),
