...
const struct Network EMPTY_NETWORK = {
        NULL, NULL, NULL, NULL,
        true,NULL, NULL,
        NULL
};
...
bool network_exists(const char *network) {
    ...
    {
        char *path;
        asprintf(&path, "/v1/projects/%s/global/networks/%s", AUTH_CONTEXT.project_id, network);
...
struct OptionalNetwork network_create(const char *network_name) {
    ...
    if (response.status_code == 200 && strlen(response.body) > 0) {
        const JSON_Value *created_json_value = if_error_exit(json_parse_string(response.body), false);
        const JSON_Object *created_json_object = json_value_get_object(created_json_value);
        ...
        if (!json_object_has_value(created_json_object, "operationType")) {
            const struct Network network = {
                    ...
                    (bool) json_object_get_boolean(created_json_object, "autoCreateSubnetworks"),
...
struct OptionalNetwork network_get(const char *network_name) {
    char *path;
    asprintf(&path, "/v1/projects/%s/global/networks/%s", AUTH_CONTEXT.project_id, network_name);
    struct ServerResponse response = gcloud_get(NULL, path, NULL);
