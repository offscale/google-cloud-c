...
bool firewall_exists(const char *firewall) {
     ...
     * GET https://compute.googleapis.com/compute/v1/projects/{project}/global/firewalls/{resourceId} */

    bool firewall_exists = false;
    {
        char *path;
        asprintf(&path, "/v1/projects/%s/global/firewalls/%s", AUTH_CONTEXT.project_id, firewall);
...
struct OptionalFirewall firewall_create(const char *network_name,
    ...
    asprintf(&body, "{\n"
                        ...
                        "  \"direction\": \"INGRESS\",\n"
                        "  \"name\": \"%s\",\n"
                        "  \"network\": ""\"https://compute.googleapis.com/compute/v1/projects/%s/global/networks/%s\"\n"
    ...
    const struct OptionalFirewall optionalFirewall = {response.status_code == 200, firewall};
